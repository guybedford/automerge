    ...
    T::Prefix: UnsignedPrefix,
{
    ...
    pub fn get_delta(&self, start: usize, pos: usize) -> Option<PrefixSeek<T::Prefix, T::Get<'_>>> {
        // FIXME
        self.iter_range(start..self.col.len()).advance_to_old(pos)
    }
}
...
impl<'a, T: PrefixValue> Iterator for PrefixIter<'a, T> {
    ...
    fn nth(&mut self, n: usize) -> Option<Self::Item> {
        ...
        //let (si, total, pos) = self.col.unwrap().col.index.find_slab_at_item(target_pos);
...
impl<'a, T: PrefixValue> PrefixIter<'a, T> {
    ...
    pub fn advance_to_old(&mut self, pos: usize) -> Option<PrefixSeek<T::Prefix, T::Get<'a>>> {
        ...
        let delta = total - T::to_prefix(value) - base;
        Some(PrefixSeek {
            pos,
            total,
            delta, //: exclusive - self.base,
            value,
        })
    }
    ...
    T::Prefix: UnsignedPrefix,
{
    /*
        pub fn advance_prefix(&mut self, n: T::Prefix) -> Option<PrefixSeek<T::Prefix, T::Get<'a>>> {
            ...
            let target_pos = self.col.unwrap().get_index_for_total(seek_target);
            self.advance_to_old(target_pos)
        }
    */
